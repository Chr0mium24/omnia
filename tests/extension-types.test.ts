...
describe('formatSummary', () => {
  describe('chrome_api', () => {
    ...
    it('formats tabs.create without URL param', () => {
      const result = formatSummary('chrome_api', { api: 'tabs', method: 'create' });
    ...
    it('formats tabs.remove without tabId param', () => {
      const result = formatSummary('chrome_api', { api: 'tabs', method: 'remove' });
      expect(result).toBe('tabs.remove');
    });

    it('formats tabs.query', () => {
      const result = formatSummary('chrome_api', { api: 'tabs', method: 'query' });
      expect(result).toBe('tabs.query');
    });

    it('formats tabs.executeScript', () => {
      const result = formatSummary('chrome_api', { api: 'tabs', method: 'executeScript' });
      expect(result).toBe('tabs.executeScript');
    });

    it('formats generic chrome API', () => {
      const result = formatSummary('chrome_api', { api: 'bookmarks', method: 'getTree' });
  ...
  describe('cdp', () => {
    it('formats CDP command with tabId', () => {
      const result = formatSummary('cdp', { method: 'Page.navigate', tabId: 5 });
      expect(result).toBe('Page.navigate tab 5');
    });

    it('formats Network.enable', () => {
      const result = formatSummary('cdp', { method: 'Network.enable', tabId: 3 });
  ...
  describe('edge cases', () => {
    it('returns "unknown" for chrome_api with non-ChromeApiCallParams', () => {
      const result = formatSummary('chrome_api', { method: 'x', tabId: 1 } as unknown as ChromeApiCallParams);
      expect(result).toBe('unknown');
    });

    it('returns "unknown" for cdp with non-CdpCallParams', () => {
      const result = formatSummary('cdp', { api: 'x', method: 'x' } as unknown as CdpCallParams);
      expect(result).toBe('unknown');
    });
  });
});
